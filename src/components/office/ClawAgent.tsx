...
export const ClawAgent = memo(function ClawAgent({
  ...
  useFrame(() => {
    ...
    const walkPhase = Math.sin(f * 0.15)

    if (groupRef.current) {
      const g = groupRef.current
      g.position.y = isWorking
        ? Math.abs(Math.sin(f * 0.14)) * 0.04
        : isError
          ? 0
          : Math.sin(f * 0.03) * 0.01

      g.rotation.y = isWorking
        ? Math.sin(f * 0.04) * 0.06
        : Math.sin(f * 0.008) * 0.04

      g.position.x = isError ? Math.sin(f * 0.28) * 0.04 : 0
    }
    ...
    if (leftLegRef.current)  leftLegRef.current.rotation.x  = isWorking ?  walkPhase * 0.18 : 0
    if (rightLegRef.current) rightLegRef.current.rotation.x = isWorking ? -walkPhase * 0.18 : 0

    if (dotMatRef.current) {
      dotMatRef.current.color.set(isError ? '#ef4444' : isWorking ? '#22c55e' : '#f59e0b')
    }
  ...
  return (
    <group position={[worldX, 0, worldZ]}>
      ...
      >
        ...
        {/* Eye highlights */}
        <mesh position={[-0.03, 0.482, 0.074]}>
          <boxGeometry args={[0.008, 0.008, 0.01]} />
          <meshBasicMaterial color="#fff" />
        </mesh>
        <mesh position={[0.05, 0.482, 0.074]}>
          <boxGeometry args={[0.008, 0.008, 0.01]} />
          <meshBasicMaterial color="#fff" />
        </mesh>
